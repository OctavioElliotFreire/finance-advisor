...
**Known code/doc mismatch as of 2026-08-09 (handoff ingestion):** this session's theme-token code (`app_colors.dart`, `app_typography.dart`, `app_theme.dart`, `pubspec.yaml`'s `google_fonts` addition) was written against the raw-mockup-only extraction *before* the handoff doc arrived, and is now confirmed **wrong**: it uses Inter+JetBrains Mono (should be Instrument Sans+Geist Mono), maps income to a green success color (should be `ink-muted`, no success color exists at all), and only has 4 member colors (should be 6 + an "Outros" bucket). Fix this code in the next implementation pass — don't build further on top of it as-is.
...
**Confirmed** per-member stacked bar (handoff §6.4: "full-height stacked bars by member") — subject to the density rulebook above (unstacked at all-selected, stacked at 2-4, ranked-list-only at 5+). `fl_chart`'s `BarChartGroupData` + stacked `BarChartRodStackItem`s covers the 2-4-member case; the 5+ case needs a plain list, not a chart. No real per-member aggregation endpoint exists yet on the backend (see the phased implementation plan) — this chart's real data wiring is backend-gated, not a frontend-only task.
