...
**Resolved 2026-08-09 (Phase 1):** the theme-token code (`app_colors.dart`, `app_typography.dart`, `app_theme.dart`, `pubspec.yaml`'s `google_fonts` addition) was originally written against the raw-mockup-only extraction, before the handoff doc arrived, and was wrong in the ways described below. It has since been corrected to match this doc: Instrument Sans + IBM Plex Mono (Geist Mono isn't in the pinned `google_fonts` catalog — see `app_typography.dart`'s doc comment), no success/positive color anywhere, 6 member accents + an `outros` fallback. See `PLAN.md`'s Milestone 12 "Phase 1 implemented" entry for the full list of what landed. pt-BR strings (Phase 2 part 1), global scope controls (Phase 2 part 2), per-member monthly-spend grouping, real Início/Contas·Saldos content, and dark-mode application are now implemented, see Changelog.
...
| 2026-08-11 | Dark mode implementation | Color Palette, Open Questions | Found the foundation (`AppTheme.dark` with the real dark hex table, `AppSemanticColors.dark`, `MaterialApp`'s `darkTheme`/`ThemeMode.system` wiring) already existed from earlier work — this pass was the narrower remaining gap: 11 call sites across `combo_cash_flow_chart.dart`, `monthly_spend_chart.dart`, `accounts_view.dart`, `family_view.dart`, `member_summary_row.dart`, `alert_row.dart`, and `member_filter_chips.dart` that bypassed the theme and referenced light-only `AppPalette` constants directly, plus `AppChartColors.income()` and `progress_track.dart`'s compile-time-const `trackColor` default. Caught and fixed a mapping mistake during manual QA before finishing: `AppPalette.inkMuted` sites were first ported to `colorScheme.onSurfaceVariant` (`inkSecondary` — a different, darker gray) instead of `colorScheme.tertiary` (the role `AppTheme` explicitly sets to `inkMuted`/`darkInkMuted`), which would have silently darkened several light-mode grays. See `PLAN.md`'s Milestone 12 "Dark mode" entry for what landed and the dark+light manual QA pass. |
| 2026-08-11 | Member-level deviation anomaly rule | Alerts — cobranças incomuns | New `detect_member_deviations` backend rule (same 30-day window/ratio logic as the existing category rule, grouped by member via `connection_member_map()`), with a real `household_member_id` column on `anomaly_flags` and a matching visibility fix so a connection-restricted member can see a flag about their own spending even without a transaction to attribute it through. Frontend got schema parity only (`householdMemberId` on the Flutter model) — the Início per-member sub-line copy and the anomaly card's "member badge" (both still unbuilt) are separate frontend work, not done here. See `PLAN.md`'s Milestone 12 "Member-level deviation anomaly rule" entry for what landed and the test coverage. |
| 2026-08-11 | Six-column Extrato table + internal-transfer netting | Data Model, §6a Table conventions, What relaxes, Screen inventory | Closed the last blocker this section itself named: the six-column table needed internal-transfer detection for its "Interna" tag, which didn't exist anywhere. New `detect_internal_transfers` (amount/direction/2-day-window matching across accounts, per this section's own spec) sets a real `Transaction.is_transfer` column, applied to the dashboard cash-flow, category-breakdown, and spending-by-member aggregates — actual netting, not just a cosmetic tag. Six-column table shipped on web (member/account resolved client-side, no schema change needed for those); mobile now explicitly drops transfer rows instead of showing them muted, per this section's own mobile-vs-web distinction. Confirmed, reasoned scope decisions: anomaly-rule baselines not updated to exclude transfers (separate, adjacent work); sticky table header simplified to a plain fixed header (documented, not silent); row-click → right-side panel still not built (separate, bigger, zero-precedent task). See `PLAN.md`'s Milestone 12 "Six-column Extrato table + internal-transfer netting" entry for what landed and the test coverage. |
| 2026-08-11 | Transaction drill-down panel | §6a What relaxes (Drill-down), Table conventions, Screen inventory | Closed the last item on the whole design-system punch list — `design.md`'s entire spec for this was one sentence ("recategorize, split, flag"), so most of this pass was making real product decisions, not implementing a spec: split ships as a genuine feature (new `transaction_splits` table, category-aware breakdown query) rather than deferred; flag reuses the existing `AnomalyFlag` system (`rule="manual"`) instead of inventing a second concept; "raw bank descriptor" and an enforced category vocabulary were both scoped out as fabricating fields/constraints that don't exist anywhere in this app's data model. New `TransactionDetailPanel` shared content widget renders as a persistent right-side panel (web, list stays visible) or a bottom sheet (mobile) — first side-panel infrastructure in the app, and the first tappable transaction row anywhere. See `PLAN.md`'s Milestone 12 "Transaction drill-down panel" entry for the full scope reasoning and test coverage. |
...
- **Internal transfers**: match on amount, direction, and a short time window across linked accounts; **net out both legs**. Without this, moving money between the household's own accounts inflates both income and spending. Pix between family members hits this at high frequency. This is the single most important correctness rule in the app. **Implemented (2026-08-11)**: `detect_internal_transfers` (backend/app/services/transfer_detection.py) — opposite-sign amounts, different accounts, within a 2-day window; a real `Transaction.is_transfer` column, applied to the dashboard cash-flow, category-breakdown, and spending-by-member aggregates. Not yet applied to the anomaly-detection rules' own baselines (a separate, adjacent correctness improvement — see `PLAN.md`).
...
| Deviation from own average | `Gasto acima da média` | Exists at both grains now — category level (`detect_category_deviations`) and, as of 2026-08-11, member level (`detect_member_deviations`, real `household_member_id` attribution + restricted-member visibility). The Início "40% acima da média dela" sub-line and a visual member badge on the anomaly card are still not built — the backend rule exists, but neither frontend surface consumes it yet (see `PLAN.md`'s Milestone 12 "Member-level deviation anomaly rule" entry) |
...
**Implemented (2026-08-10)**, with one known simplification: the bar-legend row renders `Realizado · Comprometido R$X` as plain text, not an actual visual progress bar — see `PLAN.md`'s Milestone 12 "Real Início + Contas · Saldos content" entry.
...
**Implemented (2026-08-10)** — see `PLAN.md`'s Milestone 12 "Real Início + Contas · Saldos content" entry.
...
**Partially implemented (2026-08-10)** — filter pills (minus `Parcelados`), category on the row, flag icon, and an account-grouped subheader (member dot + masked number, institution substituted with account name; not pinned/sticky, scrolls normally) all shipped; the parcela counter/filter, row-tap detail sheet, and CSV/PDF export remain out of scope (confirmed backend gaps, not just wiring — see `PLAN.md`'s Milestone 12 "Contas · Extrato polish" entry).
...
**Implemented (2026-08-10)** — see `PLAN.md`'s Milestone 12 "Web top-bar navigation" entry. Known simplifications versus the mockup: each tab screen still renders its own `AppBar` beneath the new top bar rather than one fully-merged bar (a bigger refactor, not attempted this pass), and the theme toggle/avatar are omitted (no dark-mode application or settings/user-menu exists yet to back them).
...
| Transaction rows | Two-line card | **Implemented (2026-08-11)** — six-column table (Data · Descrição · Membro · Conta · Categoria · Valor) at the wide breakpoint, backed by new internal-transfer detection (`detect_internal_transfers`) so the "Interna" muted tag is real, not cosmetic — see `PLAN.md`'s "Six-column Extrato table + internal-transfer netting" entry for what landed and its known simplifications (no true sticky-scroll header, no drill-down click). |
| Drill-down | Bottom sheet | **Implemented (2026-08-11)** — right-side panel on web (list stays visible behind it), bottom sheet on mobile, sharing one `TransactionDetailPanel` content widget. Recategorize + flag + split (net-new `transaction_splits` table + endpoint) all ship — see `PLAN.md`'s "Transaction drill-down panel" entry for the real product decisions this required (design.md only ever named the three actions, never specified them) and what got scoped down (no separate "raw bank descriptor" field, no enforced category vocabulary). |
...
- Date gets its own column; mobile's date-group headers (`16 Domingo`) don't carry over. **Implemented (2026-08-11).**
- Member column is always **dot + name**, never the dot alone. **Implemented** — resolved client-side from the account's own `owner_member_id`, no backend schema change needed.
- Category is a neutral pill (`surface-fill` bg, `ink-secondary` text) — never colored (color is reserved for member identity). **Implemented** — new `CategoryPill` widget.
- Internal transfers stay **visible but visibly muted** (`ink-muted` row, tagged `Interna`) rather than hidden, unlike mobile — width affords showing the app's own filtering work. **Implemented** — mobile now explicitly drops transfer rows instead (a real, deliberate difference, not an oversight).
- Sticky table header on scroll; row hover uses `surface-fill`; row click opens the right-side panel, never a modal. **Row hover: implemented.** **Sticky header: simplified** — a plain fixed header above the (page-scrolled) body, not a true independent-scroll-region sticky header; documented as a known simplification, not a silent shortcut. **Row click → panel: implemented (2026-08-11)** — opens `TransactionDetailPanel` as a persistent side panel (web) / bottom sheet (mobile), never a modal dialog, per this exact rule.
...
**Delivered (2026-08-11)**: the six-column Extrato table itself, backed by real internal-transfer detection/netting (`detect_internal_transfers`) applied to the dashboard cash-flow, category-breakdown, and spending-by-member aggregates — see `PLAN.md`'s "Six-column Extrato table + internal-transfer netting" entry.
**Delivered (2026-08-11)**: the right-side drill-down panel / bottom sheet, with recategorize, split (new `transaction_splits` table), and flag (reuses the existing `AnomalyFlag` system, `rule="manual"`) all working end-to-end — see `PLAN.md`'s "Transaction drill-down panel" entry.
**Still needed**: Análises·Fluxo and ·Investimentos have no web-specific layout of their own (Investimentos is separately flagged in-code as intentionally unstyled); a true sticky-scroll table header (current one is a documented simplification); the Análises·Gastos "R$ X em transferências... foram excluídos" footnote (netting now exists, but this specific UI surfacing of it doesn't yet); extending transfer-netting to the anomaly-detection rules' own baselines (a separate, adjacent correctness improvement, not done this pass); the LGPD consent screen (undesigned, separate item).
...
- **Dark mode application**: implemented (2026-08-11) — `AppTheme.dark`/`MaterialApp`'s `darkTheme`/`ThemeMode.system` wiring, plus every widget that referenced light-only `AppPalette` constants directly now reads theme-aware `colorScheme`/`semanticColors`/`AppChartColors` roles instead. No manual theme-mode toggle exists (system preference only) — add one later if requested.
